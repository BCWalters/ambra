  ...
  return (
    <div style={activities.length ? {
      ...
      <div role="status" aria-live="polite" aria-relevant="additions text">
        {activities.map(({ id, fileName, phase }) => (
          <div key={id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "4px 0" }}>
            {phase !== "complete" && <Spinner size="tiny" role="presentation" aria-hidden="true" />}
            <Body1 style={{ overflowWrap: "anywhere", minWidth: 0 }}>
              {t(PHASE_LABELS[phase], { fileName })}
            </Body1>
          </div>
        ))}
