...
import { Body1Strong, Button, OverlayDrawer, useRestoreFocusSource } from "@fluentui/react-components";
import { DismissRegular } from "@fluentui/react-icons";
import { CHROME_BORDER, CHROME_SHADOW } from "../reader/chromeTheme.js";
import { useTranslation } from "../i18n/LocaleContext.js";

export interface ModalFlyoutProps {
  open: boolean;
  title: string;
  onRequestClose: () => void;
...
export const ModalFlyout: FC<ModalFlyoutProps> = ({
  open,
  title,
  onRequestClose,
  backgroundSolid,
  ...
  return (
    <OverlayDrawer
      {...(onAfterClose ? {} : restoreFocusSource)}
      open={open}
      position="end"
      ...
      surfaceMotion={{
        onMotionFinish: (_event, data) => {
          if (data.direction === "exit") onAfterClose?.();
          if (data.direction === "enter") onAfterOpen?.();
        },
      }}
